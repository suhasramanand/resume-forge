...
export function LinkRenderer({ text }: LinkRendererProps) {
  // Match pattern: TEXT|url (for LinkedIn and GitHub)
  const linkPattern = /(LINKEDIN|GITHUB)\|([^\s|]+)/g;
  ...
  while ((match = linkPattern.exec(text)) !== null) {
    ...
    if (match.index > lastIndex) {
      ...
      // Split by | and add separators
      const textParts = beforeText.split('|');
      textParts.forEach((part, idx) => {
        if (part.trim()) {
          parts.push(part.trim());
          if (idx < textParts.length - 1) {
            parts.push(<span key={`sep-${idx}`} style={{ margin: '0 4px' }}>|</span>);
          }
        }
      });
    }
    
    // Add the link with proper icon
    const linkType = match[1];
    const url = match[2];
    const linkText = linkType === 'LINKEDIN' ? 'LinkedIn' : 'Github';
    const isLinkedIn = linkType === 'LINKEDIN';
    
    parts.push(
      <span key={match.index} style={{ display: 'inline-flex', alignItems: 'center', gap: '4px', verticalAlign: 'middle' }}>
        {isLinkedIn ? (
        ...
        <a 
          ...
          style={{ textDecoration: 'underline', textDecorationColor: '#d4d4d4', color: '#000000', verticalAlign: 'middle' }}
        >
          {linkText}
        </a>
      </span>
  ...
  if (lastIndex < text.length) {
    ...
    textParts.forEach((part, idx) => {
      if (part.trim()) {
        parts.push(part.trim());
        if (idx < textParts.length - 1) {
          parts.push(<span key={`sep-rem-${idx}`} style={{ margin: '0 4px' }}>|</span>);
        }
      }
    });
  }
  ...
  return <span style={{ display: 'inline-flex', alignItems: 'center', flexWrap: 'wrap', gap: '4px' }}>{parts}</span>;
}
